...
import { createEphemeralKeypair, makeSecureHello, NonceReplayGuard, PeerScoreBook, SecureChannel, verifySecureHello } from "./security.mjs";
...
function validRecord(record, authorityPublicKeyPem) { return record && record.issuer === hash(authorityPublicKeyPem) && record.payload.nodeId === hash(record.payload.publicKeyPem || "") && typeof record.payload.host === "string" && Number.isInteger(record.payload.port) && record.payload.port > 0 && record.payload.port < 65536 && record.payload.expiresAt > Date.now() && verifyPayload(record.payload, record.signature, authorityPublicKeyPem); }
...
export function decode(line) { if (Buffer.byteLength(line) > MAX_WIRE_BYTES) throw new Error("Wire limit exceeded"); const message = JSON.parse(line); if (!message || !["SECURE_HELLO", "SECURE_DATA"].includes(message.type)) throw new Error("Unknown outer transport message"); return message; }

export class TestnetNode extends EventEmitter {
  constructor({ name, dataDir, identity, authorityPublicKeyPem, port = 0 }) { super(); this.name = name; this.identity = identity; this.authorityPublicKeyPem = authorityPublicKeyPem; this.port = port; this.store = new StateStore(join(dataDir, `${name}.state.json`)); this.chain = this.store.load(); this.records = new Map(); this.peers = new Map(); this.sockets = new Set(); this.replayGuard = new NonceReplayGuard(); this.peerScores = new PeerScoreBook(); }
  ...
  connect(nodeId) { const record = this.records.get(nodeId); if (!record) throw new Error("Unverified peer discovery record"); if (this.peerScores.isBanned(nodeId)) throw new Error("Peer is currently banned"); const socket = net.createConnection({ host: record.payload.host, port: record.payload.port }); socket.once("connect", () => this.attach(socket, true)); return socket; }
  score(nodeId, reason, points = 25) { if (nodeId) this.peerScores.penalize(nodeId, reason, points); this.emit("peerPenalty", { nodeId, reason, points }); }
  attach(socket, outbound) { this.sockets.add(socket); let buffer = ""; let count = 0; let windowStart = Date.now(); let sent = false; let remoteId; let session; const ephemeral = createEphemeralKeypair(); const sendHello = () => { if (!sent) { socket.write(encode(makeSecureHello(this.identity, CHAIN_ID, ephemeral))); sent = true; } }; const reject = (reason, points = 25) => { this.score(remoteId, reason, points); socket.destroy(); }; const sendInner = (message) => { if (!session) throw new Error("Secure session not established"); socket.write(encode(session.channel.encrypt(message))); }; if (outbound) sendHello(); socket.setTimeout(15000, () => socket.destroy()); socket.on("data", (chunk) => { if (chunk.length + Buffer.byteLength(buffer) > MAX_WIRE_BYTES * 2) return reject("wire-buffer-limit", 50); buffer += chunk.toString("utf8"); const lines = buffer.split("\n"); buffer = lines.pop(); for (const line of lines) { const now = Date.now(); if (now - windowStart > 1000) { windowStart = now; count = 0; } if (++count > 60) return reject("rate-limit", 50); try { const message = decode(line); if (message.type === "SECURE_HELLO") { if (!verifySecureHello(message, CHAIN_ID)) return reject("invalid-secure-hello", 75); remoteId = message.nodeId; const record = this.records.get(remoteId); if (!record || record.payload.publicKeyPem !== message.publicKeyPem) return reject("unverified-peer", 100); if (this.peerScores.isBanned(remoteId)) return reject("banned-peer", 100); if (!this.replayGuard.accept(remoteId, message.nonce)) return reject("replayed-hello", 100); session = { socket, channel: new SecureChannel({ localNodeId: this.identity.nodeId, remoteNodeId: remoteId, localEphemeral: ephemeral, remoteEphemeralPublicKeyPem: message.ephemeralPublicKeyPem, chainId: CHAIN_ID }) }; this.peers.set(remoteId, session); socket.setTimeout(0); sendHello(); this.peerScores.reward(remoteId); this.emit("peer", remoteId); } else if (message.type === "SECURE_DATA") { if (!session) return reject("data-before-authentication", 75); const inner = session.channel.decrypt(message); if (inner.type === "BLOCK") { const result = this.chain.add(inner.block); if (result.accepted) { this.store.save(this.chain); this.emit("block", inner.block, result); } } else if (inner.type === "PING") sendInner({ type: "STATUS", height: this.chain.height, tip: this.chain.tip }); else if (inner.type !== "STATUS") return reject("unknown-secure-message", 50); } } catch { return reject("malformed-or-unauthenticated-message", 75); } } }); socket.on("error", () => undefined); socket.on("close", () => { this.sockets.delete(socket); for (const [id, peer] of this.peers) if (peer.socket === socket) this.peers.delete(id); }); }
  mineEmpty(timestamp) { const block = mine({ parent: this.chain.tipNode.block, chain: this.chain.main, timestamp }); const result = this.chain.add(block); if (!result.accepted) throw new Error("Locally mined block rejected"); this.store.save(this.chain); for (const peer of this.peers.values()) peer.socket.write(encode(peer.channel.encrypt({ type: "BLOCK", block }))); return block; }
