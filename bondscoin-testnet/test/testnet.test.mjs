...
test("authenticated peers relay an empty no-value testnet block", async () => {
  ...
  for (let i = 0; i < 30 && (!alice.peers.size || !bob.peers.size); i += 1) await sleep(25); assert.equal(alice.peers.size, 1); assert.equal(bob.peers.size, 1); alice.mineEmpty(1735689605000); for (let i = 0; i < 30 && bob.height === 0; i += 1) await sleep(25); assert.equal(bob.height, 1); await alice.stop(); await bob.stop();
});
